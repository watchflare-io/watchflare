...
**Branch naming:** `<type>/<short-description>` matching the Conventional Commits prefix (`feat/discord-alerts`, `fix/scrollbar-overflow`, `docs/contributing-update`). Branch from and target `main`. The maintainer may commit small changes directly to `main`; feature work uses a dedicated branch.
