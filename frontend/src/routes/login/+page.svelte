...
<div class="flex min-h-dvh items-center justify-center bg-background p-4">
	<div class="w-full max-w-md">
		...
		<div class="mb-8 text-center">
			...
			<p class="text-sm text-muted-foreground">Host Monitoring Dashboard</p>
		</div>

		<!-- Login Card -->
		<div class="rounded-lg border bg-card p-4 sm:p-8 shadow-sm">
			{#if step === 'credentials'}
				<h2 class="text-lg font-semibold text-foreground mb-6">Login to your account</h2>
				<form
					onsubmit={(e) => {
						e.preventDefault();
						handleLogin();
					...
					<div class="mb-4">
						...
						<input
							...
							class="w-full rounded-lg border bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-primary disabled:opacity-50 {fieldErrors.email
								...
								: ''}"
						/>
						{#if fieldErrors.email}<p id="email-error" class="mt-1 text-xs text-destructive">
								{fieldErrors.email}
							</p>{/if}
					</div>
					<!-- Password -->
					<div class="mb-6">
						...
						<input
							...
							class="w-full rounded-lg border bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-primary disabled:opacity-50 {fieldErrors.password
								...
								: ''}"
						/>
						{#if fieldErrors.password}<p id="password-error" class="mt-1 text-xs text-destructive">
				...
				>
					<div class="mb-6">
						...
						<input
							...
							oninput={() => {
								if (!useBackupCode && totpCode.length === 6) handleVerifyTOTP();
							}}
		...
		<!-- Footer -->
		<p class="mt-6 text-center text-xs text-muted-foreground">Watchflare Host Monitoring</p>
	</div>
</div>
