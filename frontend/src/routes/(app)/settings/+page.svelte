	...
	>
		...
		<div>
			...
			<p class="mt-0.5 text-warning/80">
				Serve Watchflare over HTTPS to enable secure cookies. Either configure direct TLS via <code
					class="font-mono text-xs bg-warning/10 px-1 rounded">TLS_CERT_FILE</code
				>
				/
				<code class="font-mono text-xs bg-warning/10 px-1 rounded">TLS_KEY_FILE</code>, or use a
				reverse proxy (Nginx, Caddy, Traefik) and add its IP to the
				<code class="font-mono text-xs bg-warning/10 px-1 rounded">TRUSTED_PROXIES</code> environment
				variable.
			</p>
		</div>
	</div>
...
<div class="rounded-lg border bg-card p-4 sm:p-6 mb-6">
	...
	<div class="mb-6">
		...
		<p class="text-xs text-muted-foreground mb-3">
			Percentage values that trigger warning and critical colors in CPU, memory and disk gauges
		</p>
