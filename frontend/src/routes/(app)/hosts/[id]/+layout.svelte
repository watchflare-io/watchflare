		...
		if (event.type === 'host_update') {
			...
			if (host) {
				...
				host = {
					...
					ignore_ip_mismatch: update.ignore_ip_mismatch ?? host.ignore_ip_mismatch,
...
{#if loading}
	...
	<div class="flex gap-3 mb-6 animate-pulse overflow-x-auto">
		{#each Array(5) as _}
			<div class="shrink-0 rounded-lg border bg-card px-3 py-2 flex items-center gap-2">
				<div class="h-3.5 w-3.5 rounded bg-muted"></div>
		...
		<div class="mb-6 rounded-lg border border-warning bg-warning/10 p-4 space-y-3">
			<div class="flex items-center justify-between gap-4 flex-wrap">
				<p class="text-sm font-medium text-warning">
					This token is valid for 24 hours and will not be displayed again. Make sure to copy it or
					use it now.
				</p>
