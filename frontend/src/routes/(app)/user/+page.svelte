<script lang="ts">
	...
	async function handleChangePassword() {
		...
		try {
			...
			error =
				message === 'current password is incorrect' ? 'Current password is incorrect.' : message;
		} finally {
			loading = false;
		}
	}
...
<div class="max-w-2xl space-y-6">
	<!-- Header -->
	<div class="mb-6">
		<h1 class="text-xl sm:text-2xl font-semibold text-foreground">Account</h1>
		<p class="text-sm text-muted-foreground mt-1">Manage your profile and credentials</p>
	</div>
		...
		<div class="mb-5">
			...
			>
				<input
					...
					value={editUsername}
					oninput={(e) => {
						usernameOverride = (e.target as HTMLInputElement).value;
					}}
		...
		<div>
			...
			>
				<input
					...
					value={editEmail}
					oninput={(e) => {
						emailOverride = (e.target as HTMLInputElement).value;
					}}
		...
		>
			...
			{#if !regenDone}
				<p class="text-sm text-muted-foreground mb-4">
					Enter your authenticator code to generate new backup codes. Old codes will be invalidated.
				</p>
				<input
					type="text"
					inputmode="numeric"
			...
			{:else}
				...
				<label class="flex items-start gap-2 mb-6 cursor-pointer">
					<input type="checkbox" bind:checked={regenSaved} class="mt-0.5 rounded border-border" />
