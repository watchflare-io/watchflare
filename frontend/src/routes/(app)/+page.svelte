<script lang="ts">
	...
	let packagesStats = $state({
		outdatedCount: 0,
		securityCount: 0,
		outdatedHostsCount: 0,
		securityHostsCount: 0
	});
	...
	let firstMetric = $derived($aggregatedMetrics.length >= 2 ? $aggregatedMetrics[0] : null);
	...
	let hasSufficientTrendData = $derived(
		(() => {
			...
			const actualMs =
				new Date(lastMetric.timestamp).getTime() - new Date(firstMetric.timestamp).getTime();
			return actualMs >= TIME_RANGE_MS[selectedTimeRange] * TREND_COVERAGE_THRESHOLD;
		})()
	);

	let trends = $derived({
		cpu: getTrend(lastMetric?.cpu_usage_percent ?? null, firstMetric?.cpu_usage_percent ?? null),
		memory: getTrend(
		...
		loadAvg: getTrend(lastMetric?.load_avg_1min ?? null, firstMetric?.load_avg_1min ?? null)
	});
			...
			<p class="text-sm text-muted-foreground mt-1">
				...
				{:else}
					Global uptime at <span class="font-medium text-foreground"
						>{formatPercent((stats.onlineHosts / stats.totalHosts) * 100)}</span
					> in the last 24h
				{/if}
			</p>
		</div>
