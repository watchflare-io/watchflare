<script lang="ts">
	...
	const hasActiveFilters = $derived(searchTerm !== '' || isStatusFiltered || isManagerFiltered);
	...
	function handleSort(column: string) {
		if (sortColumn === column) {
			...
			sortColumn = column;
			sortOrder = column === 'host_count' || column === 'available_version' ? 'desc' : 'asc';
		}
		offset = 0;
		updateURL();
		loadData();
	...
	onMount(() => {
		...
		selectedStatuses = new Set(params.getAll('status').filter(Boolean) as GlobalPackageStatus[]);
...
<div class="mb-6">
	...
	<p class="text-sm text-muted-foreground mt-0.5">Inventory of all installed packages</p>
</div>
...
<div class="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4 lg:gap-4">
	{#if initialLoading}
		{#each Array(4) as _}
			<div class="flex items-center gap-2.5 rounded-lg border bg-card px-3 py-2.5 animate-pulse">
	...
	{:else}
		...
		>
			...
			<div class="min-w-0">
				<p class="text-xs text-muted-foreground truncate">Outdated packages</p>
				<p class="text-sm font-semibold {outdatedCount > 0 ? 'text-warning' : 'text-foreground'}">
		...
		>
			...
			<div class="min-w-0">
				<p class="text-xs text-muted-foreground truncate">Security updates</p>
				<p
					class="text-sm font-semibold {securityCount > 0 ? 'text-destructive' : 'text-foreground'}"
				>
		...
		<div class="flex items-center gap-2.5 rounded-lg border bg-card px-3 py-2.5">
			...
			<div class="min-w-0">
				<p class="text-xs text-muted-foreground truncate">Outdated hosts</p>
				<p
	...
	<DropdownMenu.Root>
		...
		<DropdownMenu.Content align="start">
			{#each [{ value: 'security' as StatusFilter, label: 'Security update' }, { value: 'outdated' as StatusFilter, label: 'Outdated' }, { value: 'up_to_date' as StatusFilter, label: 'Up to date' }, { value: 'not_checked' as StatusFilter, label: 'Not checked' }] as status}
				<DropdownMenu.Item closeOnSelect={false} onclick={() => toggleStatus(status.value)}>
					<div
		...
		<DropdownMenu.Trigger>
			{#snippet child({ props })}
				...
				>
					...
					<span class="hidden sm:inline">{managerFilterLabel}</span>
		...
		<DropdownMenu.Content align="start">
			<div class="max-h-48 overflow-y-auto">
				{#each availableManagers as manager}
					<DropdownMenu.Item closeOnSelect={false} onclick={() => toggleManager(manager)}>
						<div
							class="flex h-4 w-4 shrink-0 items-center justify-center rounded border
						...
						<span class="flex-1">{getManagerLabel(manager)}</span>
					</DropdownMenu.Item>
				{/each}
				{#if availableManagers.length === 0}
					<DropdownMenu.Item disabled>
						<span class="text-muted-foreground text-xs">No packages yet</span>
					</DropdownMenu.Item>
				{/if}
			</div>
			{#if isManagerFiltered}
					...
					>
						...
						>
							<button
								...
								'available_version'
									? 'bg-table-header-active text-foreground'
									: ''}">Latest Version {@render sortIcon('available_version')}</button
							>
						</th>
					</tr>
				</thead>
				<tbody
					class="divide-y divide-border {tableLoading ? 'opacity-50 pointer-events-none' : ''}"
				>
					{#each packages as pkg}
						<tr class="hover:bg-muted/20 transition-colors">
							...
							<td class="px-2 py-3 text-sm font-mono whitespace-nowrap">
