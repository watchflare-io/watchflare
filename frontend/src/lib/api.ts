...
export class ApiError extends Error {
	constructor(
		...
		super(capitalizeFirst(data?.error || data?.message || statusText || 'API request failed'));
		this.name = 'ApiError';
	}
...
export async function enableTOTP(code: string): Promise<TOTPEnableResponse> {
	return apiRequest<TOTPEnableResponse>('/2fa/enable', {
		method: 'POST',
...
export async function regenerateBackupCodes(code: string): Promise<TOTPRegenerateResponse> {
	return apiRequest<TOTPRegenerateResponse>('/2fa/backup-codes/regenerate', {
		method: 'POST',
		body: JSON.stringify({ code })
	});
}
...
export async function renameHost(id: string, newName: string): Promise<{ message: string }> {
...
export async function getSensorReadings(
	...
	return apiRequest<GetSensorReadingsResponse>(`/hosts/${hostId}/sensor-readings${query}`);
}
...
export async function getContainerMetrics(
	...
	const query = buildQueryString({ time_range: timeRange });
	return apiRequest<GetContainerMetricsResponse>(`/hosts/${hostId}/container-metrics${query}`);
}
...
export async function getAggregatedMetrics(
	...
	const query = buildQueryString({ time_range: timeRange });
	return apiRequest<GetAggregatedMetricsResponse>(`/hosts/metrics/aggregated${query}`);
}
...
export async function getPackageStats(hostId: string): Promise<GetPackageStatsResponse> {
...
export async function getPackageCollections(
	...
	return apiRequest<GetPackageCollectionsResponse>(`/hosts/${hostId}/packages/collections${query}`);
}
...
export async function getPackageHistory(
	...
	return apiRequest<GetPackageHistoryResponse>(`/hosts/${hostId}/packages/history${query}`);
}
...
export async function triggerPackageCollect(
	...
	return apiRequest<{ message: string; command_id: string }>(`/hosts/${hostId}/packages/collect`, {
		method: 'POST'
	});
}
...
export async function testSmtpConnection(recipient?: string): Promise<{ message: string }> {
...
export async function testNotificationChannelDraft(url: string): Promise<{ message: string }> {
