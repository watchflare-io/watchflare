<script lang="ts">
	...
	type NavItem = { type: 'host'; id: string } | { type: 'package'; name: string };
	...
	function openPalette() {
		...
		if (quickHosts.length === 0) {
			api
				.listHosts({ perPage: QUICK_HOSTS_LIMIT })
				.then((r) => {
					quickHosts = r.hosts ?? [];
				})
				.catch(() => {});
	...
	function handleInput() {
		...
		searchTimeout = setTimeout(async () => {
			try {
				...
				hostResults = hostsResult.status === 'fulfilled' ? (hostsResult.value.hosts ?? []) : [];
				packageResults =
					packagesResult.status === 'fulfilled' ? (packagesResult.value.packages ?? []) : [];
			} finally {
				loading = false;
			}
		}, 200);
							...
							>
								...
								<div class="min-w-0 flex-1 text-left">
									<div class="flex items-center gap-2">
										...
										<span class="h-1.5 w-1.5 shrink-0 rounded-full {getStatusDot(host.status)}"
										></span>
									</div>
									{#if host.hostname}
		...
		{#if query.trim()}
			<div class="flex items-center gap-4 border-t px-3.5 py-2">
				...
				<span class="hidden sm:inline-flex items-center gap-1.5 text-[11px] text-muted-foreground">
					<kbd class="rounded border bg-background px-1 py-px font-mono text-[10px] leading-relaxed"
						>Esc</kbd
					>
					Close
				</span>
				{#if resultCount > 0}
					<span class="ml-auto font-mono text-[11px] text-muted-foreground/60">
						{#if hostResults.length > 0 && packageResults.length > 0}
							{fmtCount(hostResults.length, SEARCH_HOSTS_LIMIT)} host{hostResults.length !== 1
								? 's'
								: ''} · {fmtCount(packageResults.length, SEARCH_PACKAGES_LIMIT)} package{packageResults.length !==
							1
								? 's'
								: ''}
