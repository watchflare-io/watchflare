            ...
            <div class="px-6 py-5">
                ...
                {#if step === 'qr'}
                    ...
                    <div class="flex justify-center mb-4">
                        {#if loading}
                            <div class="h-[200px] w-[200px] rounded bg-muted animate-pulse"></div>
                        {:else}
                            <canvas bind:this={canvasEl} class="rounded border"></canvas>
                        {/if}
                    </div>
                    <p class="text-xs text-muted-foreground text-center mb-1">Or enter the code manually:</p>
                    <p class="text-xs font-mono text-center text-foreground tracking-wider select-all bg-muted rounded px-3 py-2 mb-6">
                        {formatSecret(secret)}
                    </p>
                    <button
                        onclick={() => { step = 'verify'; }}
                ...
                {:else if step === 'verify'}
                    ...
                    <form onsubmit={(e) => { e.preventDefault(); handleEnable(); }}>
                        <div class="mb-6">
                            <label for="confirm-code" class="block text-sm font-medium text-foreground mb-2">
                                Verification code
                            </label>
                            <input
                                id="confirm-code"
