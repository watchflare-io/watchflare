<script lang="ts">
		...
		return {
			...
			memory:
				latest.memory_total_bytes > 0
					...
					: 0,
			disk:
				latest.disk_total_bytes > 0 ? (latest.disk_used_bytes / latest.disk_total_bytes) * 100 : 0,
	...
	const sortedHosts = $derived(() => {
		...
		const metricsMap = new Map(filtered.map((h) => [h.host.id, getLastMetrics(h.host.id)]));
...
{#snippet statusDot(host: Host, size: 'sm' | 'md' = 'md')}
	{@const isOnline = host.status === 'online' || host.status === 'ip_mismatch'}
					...
					<div class="flex items-center justify-between gap-2">
						<div class="min-w-0">
							<div class="flex items-center gap-2">
								<span class="mt-0.75">{@render statusDot(host, 'sm')}</span>
								<span class="font-medium text-foreground break-all">{host.display_name}</span>
							...
							{#if host.hostname || host.ip_address_v4 || host.configured_ip}
								<p class="text-xs text-muted-foreground mt-0.5 ml-4 flex items-center gap-2">
									{#if host.hostname}<span>{host.hostname}</span>{/if}
							...
							>
								<DropdownMenu.Root>
									...
									<DropdownMenu.Content side="bottom" align="end">
										<DropdownMenu.Item onclick={() => onRename(host)}>
											<Pencil class="h-4 w-4" />
											Rename
										</DropdownMenu.Item>
										{#if host.status !== 'pending'}
											{#if host.status === 'paused'}
												<DropdownMenu.Item onclick={() => onResume(host.id)}>
													<Play class="h-4 w-4" />
													Resume
												</DropdownMenu.Item>
											{:else}
												<DropdownMenu.Item onclick={() => onPause(host.id)}>
													<Pause class="h-4 w-4" />
													Pause
												</DropdownMenu.Item>
											{/if}
										...
										{#if onUpdateAgent && host.status === 'online' && isAgentOutdated(host.agent_version, latestAgentVersion)}
											<DropdownMenu.Item onclick={() => onUpdateAgent(host.id)}>
					...
					{#if metrics.hasData}
						<div class="space-y-2 text-xs">
							{#each [{ label: 'CPU', value: metrics.cpu }, { label: 'Mem', value: metrics.memory }, { label: 'Disk', value: metrics.disk }] as { label, value }}
								<div class="flex items-center gap-2">
									<span class="w-16 text-muted-foreground shrink-0">{label}</span>
							...
							{/each}
							<div class="flex items-center gap-2">
								<span class="w-16 text-muted-foreground shrink-0">Load Avg</span>
							...
							<div class="flex items-center gap-2">
								<span class="w-16 text-muted-foreground shrink-0">Net</span>
								<span class="text-foreground"
									>↓{formatBytes(metrics.netRx)}/s ↑{formatBytes(metrics.netTx)}/s</span
								>
							</div>
							{#if metrics.temp > 0}
								<div class="flex items-center gap-2">
									<span class="w-16 text-muted-foreground shrink-0">Temp</span>
									<span class="text-foreground">{Math.round(metrics.temp)}°C</span>
								</div>
							{/if}
							{#if host.agent_version}
								<div class="flex items-center gap-2">
									<span class="w-16 text-muted-foreground shrink-0">Agent</span>
									<span class="text-foreground flex items-center gap-1">
										v{host.agent_version}
										{#if updatingHosts?.has(host.id)}
											<span class="inline-flex" title="Updating to v{latestAgentVersion}…">
												<Loader2 class="h-3.5 w-3.5 animate-spin text-warning" />
											</span>
										{:else if isAgentOutdated(host.agent_version, latestAgentVersion)}
											<span class="text-warning font-medium">↑</span>
										{/if}
									</span>
								</div>
							{/if}
							{#if (host.outdated_count ?? 0) > 0 || (host.security_count ?? 0) > 0}
								<div class="flex items-center gap-2">
									<span class="w-16 text-muted-foreground shrink-0">Pkgs</span>
									<span class="flex items-center gap-1">
										{#if (host.security_count ?? 0) > 0}
											<span
												...
												title="{host.security_count} security update{(host.security_count ?? 0) > 1
													? 's'
										...
										{#if (host.outdated_count ?? 0) > 0}
											<span
												...
												title="{host.outdated_count} outdated package{(host.outdated_count ?? 0) > 1
													? 's'
			...
			>
				{#each displayedHosts as { host }}
					...
					>
						...
						<td class="px-4 py-3">
							<div class="group flex flex-col">
								<div class="flex items-center gap-2.5">
									<span class="mt-0.75">{@render statusDot(host)}</span>
									<span
								...
								{#if host.hostname}
									<span class="text-xs text-muted-foreground whitespace-nowrap ml-5"
										>{host.hostname}</span
									>
								{/if}
								{#if host.ip_address_v4 || host.configured_ip}
						...
						<td class="px-4 py-3 text-center">
							{#if metrics.hasData}
								<div class="flex flex-col items-center text-sm whitespace-nowrap">
									<span class="text-foreground">{formatBytes(metrics.netRx)}/s ↓</span>
									<span class="text-foreground">{formatBytes(metrics.netTx)}/s ↑</span>
								</div>
							{:else}
								<span class="text-muted-foreground">-</span>
							{/if}
						...
						<td class="px-4 py-3 text-center">
							{#if metrics.hasData && metrics.temp > 0}
								<span class="text-foreground">{Math.round(metrics.temp)}°C</span>
							{:else}
								<span class="text-muted-foreground">{metrics.hasData ? '' : '-'}</span>
							{/if}
						</td>
							...
							<div class="flex items-center gap-1.5">
								...
								{:else if isAgentOutdated(host.agent_version, latestAgentVersion)}
									<span
										...
										title="v{latestAgentVersion} available">↑</span
									>
								{/if}
							</div>
						</td>
	...
	{#if displayedHosts.length === 0}
		<div class="flex flex-col items-center justify-center py-12 text-center">
			...
			{#if hosts.length === 0}
				...
				<p class="text-xs text-muted-foreground mt-1">Add your first host to start monitoring</p>
			{:else}
