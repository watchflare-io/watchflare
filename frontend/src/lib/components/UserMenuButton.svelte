<script lang="ts">
	...
	const initials = $derived(displayName ? displayName.substring(0, 2).toUpperCase() : '??');
	...
	<DropdownMenu.Trigger>
		{#snippet child({ props })}
			...
			>
				...
				<span class="whitespace-nowrap overflow-hidden text-left truncate ml-3 {textClass}">
					{displayName || 'User'}
				</span>
			</button>
		{/snippet}
