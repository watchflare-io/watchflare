<script lang="ts">
    ...
    function extractRetryAfter(err: unknown): number | null {
        if (err instanceof ApiError) {
            const v = (err.data as { retry_after_seconds?: number } | undefined)
                ?.retry_after_seconds;
    ...
    const drawerCooldownLeft = $derived(
        drawerTestKey() ? cooldownSecondsLeft(drawerTestKey()) : 0,
    );

    const testDisabled = $derived(
        drawerTesting ||
            drawerCooldownLeft > 0 ||
            (formUrl.trim().length === 0 && !editingChannel),
    );
...
>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-border">
                    {#each channels as channel (channel.id)}
                        ...
                        <tr class="transition-opacity duration-200 {channel.enabled ? 'opacity-100' : 'opacity-60'}">
                            ...
                            <td class="px-3 py-2 whitespace-nowrap text-right">
                                <div class="inline-flex items-center gap-1">
                                    <button
                                        ...
                                        title={cooldown > 0 ? `Cooldown active, retry in ${cooldown}s` : 'Send a test notification'}
                        ...
                        {#if result?.text}
                            <tr>
                                <td colspan="4" class="px-3 pb-2 text-xs {result.ok ? 'text-success' : 'text-destructive'}">
                                    {#if result.ok}
    ...
    {:else if loaded}
        <p class="mt-5 text-sm text-muted-foreground">No notification channels yet. Click <span class="font-medium text-foreground">Add channel</span> to create one.</p>
    {/if}
</div>
    ...
    <div class="flex-1 overflow-y-auto px-5 py-4">
        <div class="mb-4">
            ...
            <input
                ...
                bind:value={formName}
                disabled={saving}
                class="w-full rounded-lg border bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus-visible:ring-2 disabled:opacity-50 {formNameError ? 'border-destructive focus-visible:ring-destructive' : 'focus-visible:ring-primary'}"
            />
            {#if formNameError}
        ...
        <div class="mb-4">
            ...
            <textarea
                id="channel-url"
                rows="3"
                placeholder={editingChannel ? 'Leave empty to keep the current URL' : 'discord://TOKEN@WEBHOOK_ID'}
                bind:value={formUrl}
                disabled={saving}
                class="w-full rounded-lg border bg-background px-3 py-2 text-sm text-foreground font-mono placeholder:text-muted-foreground placeholder:font-mono focus:outline-none focus-visible:ring-2 disabled:opacity-50 {formUrlError ? 'border-destructive focus-visible:ring-destructive' : 'focus-visible:ring-primary'}"
            ></textarea>
            {#if formUrlError}
            ...
            {:else}
                <details class="mt-1.5">
                    ...
                    <p class="mt-2 text-xs text-muted-foreground">
                        Full list: <a href="https://shoutrrr.nickfedor.com/services/overview/" target="_blank" rel="noopener noreferrer" class="text-primary underline">Shoutrrr documentation</a>
                    </p>
                </details>
            {/if}
        </div>
        ...
        <div class="mb-4 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-muted-foreground">Disable to keep the channel without firing notifications.</p>
            </div>
        </div>

        {#if drawerTestMessage}
            <div class="mt-4 rounded-lg border px-3 py-2 text-sm {drawerTestMessage.ok ? 'border-success/40 bg-success/10 text-success' : 'border-destructive/40 bg-destructive/10 text-destructive'}">
...
>
    <p class="text-sm text-muted-foreground">
        {#if pendingDelete}
            <span class="font-medium text-foreground">{pendingDelete.name}</span> will be removed. Future alerts will no longer be sent there.
        {/if}
    </p>
</ConfirmDialog>
...
{#if toast}
    <div
        ...
        role="alert"
        class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 rounded-lg border bg-background px-4 py-3 text-sm shadow-lg w-max max-w-[calc(100vw-2rem)] {toast.ok ? 'border-success/40 text-success' : 'border-destructive/40 text-destructive'}"
    >
        {#if toast.ok}
