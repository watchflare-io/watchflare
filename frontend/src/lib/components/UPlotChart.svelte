<script lang="ts">
	...
	function oklchToHex(L: number, C: number, H: number): string {
		...
		const bl = -0.0041960863 * l_ ** 3 - 0.7034186147 * m_ ** 3 + 1.707614701 * s_ ** 3;
		...
		return (
			'#' + [toInt(rl), toInt(gl), toInt(bl)].map((n) => n.toString(16).padStart(2, '0')).join('')
		);
	}
				...
				draw: [
					(u: uPlot) => {
						...
						for (let si = 1; si < u.series.length; si++) {
							...
							const color = (s as any)._stroke ?? (s.stroke as string) ?? '#888';
							...
							for (let i = 0; i < vals.length; i++) {
								...
								const prevGap =
									i === 0 || vals[i - 1] == null || timestamps[i] - timestamps[i - 1] > threshold;
								const nextGap =
									i === vals.length - 1 ||
									vals[i + 1] == null ||
									timestamps[i + 1] - timestamps[i] > threshold;
								if (!prevGap || !nextGap) continue;

								const cx = Math.round(u.valToPos(timestamps[i], 'x', true));
								const cy = Math.round(u.valToPos(vals[i] as number, s.scale!, true));
	...
	const gapsFn = (
		...
		for (let i = idx0 + 1; i <= idx1; i++) {
			...
			if (timestamps[i] - timestamps[i - 1] > threshold) {
				const leftPx = Math.round(u.valToPos(timestamps[i - 1], 'x', true));
				const rightPx = Math.round(u.valToPos(timestamps[i], 'x', true));
	...
	function buildOpts(width: number, chartHeight: number): uPlot.Options {
				...
				? {
						x: {
							range: (): uPlot.Range.MinMax => {
								...
								return [now - TIME_RANGE_SECONDS[timeRange!], now];
							}
						}
					}
				: {};
		...
		return {
			...
			axes: mergedAxes,
			scales: mergedScales,
			plugins: [tooltipPlugin(), cursorOverlayPlugin(), isolatedPointsPlugin()]
		};
	}
