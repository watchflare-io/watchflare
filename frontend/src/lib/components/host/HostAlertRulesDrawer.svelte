<script lang="ts">
	...
	async function loadRules() {
		...
		loadError = false;
		try {
			const [hostRes, globalRes] = await Promise.all([getHostAlertRules(hostId), getAlertRules()]);
	...
	async function autoSave() {
		...
		const invalidDuration = rules.find(
			(r) => r.dirty && r.enabled && (isNaN(r.duration_minutes) || r.duration_minutes === null)
		);
		if (invalidDuration) {
						...
						<div class="flex items-center justify-between gap-4">
							<div class="flex items-center gap-2 min-w-0">
								...
								{#if isReallyDifferent(rule)}
									<span
										class="shrink-0 text-[10px] font-medium text-primary bg-primary/10 rounded-full px-1.5 py-0.5"
										>Custom</span
									>
								{/if}
							</div>
							<div class="flex items-center gap-2 shrink-0">
								{#if isReallyDifferent(rule)}
									...
									>
										<RotateCcw
											class="h-3.5 w-3.5 {resettingRule === rule.metric_type ? 'animate-spin' : ''}"
										/>
									</button>
								{/if}
								<Toggle bind:checked={rule.enabled} onchange={() => markDirty(rule.metric_type)} />
							</div>
						</div>
