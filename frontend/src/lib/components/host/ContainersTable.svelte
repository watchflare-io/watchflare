<script lang="ts">
	...
	let sortColumn = $state<
		'name' | 'status' | 'health' | 'image' | 'cpu' | 'memory' | 'network' | 'ports'
	>('name');
	...
	const latestContainers = $derived(
		(() => {
			...
			return [...byId.values()].map(({ metric, image, runtime, ports }) => ({
				...metric,
				image,
				runtime,
				ports
...
<div class="rounded-xl border bg-card overflow-hidden">
	...
	<div class="bg-table-header px-4 py-2.5 border-b flex items-center justify-between gap-4">
		<div class="flex items-center gap-2 shrink-0">
			...
			<span
				class="inline-flex items-center justify-center rounded-full bg-muted px-2 py-0.5 text-xs font-medium text-muted-foreground"
			>
	...
	<div class="sm:hidden p-3 flex flex-col gap-2">
		{#each displayedContainers as container (container.container_id)}
			...
			<div
				...
				onkeydown={(e) =>
					(e.key === 'Enter' || e.key === ' ') && (e.preventDefault(), handleRowClick(container))}
				role="button"
				tabindex="0"
			>
				<div
				...
				<div class="px-4 py-2.5 flex flex-col gap-1">
					...
					<div class="flex items-center gap-2">
						...
						<span class="w-12 text-sm text-muted-foreground text-left shrink-0"
							>{container.cpu_percent.toFixed(1)}%</span
						>
					</div>
					<div class="flex items-center gap-2">
						...
						{/if}
						<span class="w-12 text-sm text-muted-foreground text-left shrink-0"
							>{formatBytes(container.memory_used_bytes)}</span
						>
					</div>
					<div class="flex items-baseline gap-2">
						<span class="w-16 shrink-0 text-xs text-muted-foreground">Network</span>
						<span class="text-sm font-mono"
							>↓ {formatRate(container.network_rx_bytes_per_sec, networkUnit)} ↑ {formatRate(
			...
			<tbody class="divide-y">
				{#each displayedContainers as container (container.container_id)}
					...
					<tr
						...
						onkeydown={(e) =>
							(e.key === 'Enter' || e.key === ' ') &&
							(e.preventDefault(), handleRowClick(container))}
						tabindex="0"
						role="button"
					>
